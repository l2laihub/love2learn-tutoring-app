/**
 * Edge Function: Send Enrollment Rejection Email
 *
 * Sends an email to a parent when their group session enrollment request is rejected.
 * Called via application layer when a session_enrollment status changes to 'rejected'.
 */
...
interface EnrollmentRejectionRequest {
  parent_id: string;
  student_name: string;
  subject: string;
  session_date: string;
...
serve(async (req: Request) => {
  ...
  try {
    ...
    // Parse request body
    const requestData: EnrollmentRejectionRequest = await req.json();
    const { parent_id, student_name, subject, session_date, session_time, tutor_response } = requestData;
    ...
    const emailResponse = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        ...
        to: [parentData.email],
        subject: `Group Session Enrollment Declined - ${student_name}'s ${formattedSubject} Lesson`,
        html: `
          <!DOCTYPE html>
          <html>
          <head>
            ...
            <title>Enrollment Declined</title>
          </head>
          <body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; line-height: 1.6; color: #1B3A4B; max-width: 600px; margin: 0 auto; padding: 20px; background-color: #F8FAFB;">
            <div style="background: #FFFFFF; border-radius: 14px; padding: 30px; box-shadow: 0 4px 8px rgba(27, 58, 75, 0.08);">
              ...
              <div style="background: linear-gradient(135deg, #E53935 0%, #EF5350 100%); border-radius: 12px; padding: 25px; margin-bottom: 30px; color: white; text-align: center;">
                ...
                <h2 style="margin: 0 0 5px 0; font-size: 22px;">Enrollment Declined</h2>
                <p style="margin: 0; opacity: 0.95; font-size: 14px;">Your enrollment request could not be accommodated</p>
              </div>
              ...
              <p style="color: #4A6572; font-size: 15px;">
                We regret to inform you that ${student_name}'s enrollment request for the group ${formattedSubject.toLowerCase()} session has been declined.
              </p>

              <!-- Session Details -->
              <div style="background: #FFEBEE; border-radius: 10px; padding: 20px; margin: 20px 0;">
                <h3 style="margin: 0 0 15px 0; color: #C62828; font-size: 16px;">Requested Session:</h3>
              ...
              <div style="background: #E8F5E9; border-radius: 10px; padding: 20px; margin: 25px 0; border-left: 4px solid #7CB342;">
                ...
                <ul style="margin: 0; padding-left: 20px; color: #1B3A4B;">
                  <li style="margin-bottom: 8px;">Browse other available group sessions in the app</li>
                  <li style="margin-bottom: 8px;">Request a drop-in lesson for an alternative time</li>
                  <li style="margin-bottom: 0;">Contact your tutor directly to discuss options</li>
                </ul>
              </div>
    ...
    if (!emailResponse.ok) {
      ...
      return new Response(
        JSON.stringify({ error: 'Failed to send enrollment rejection email', details: errorData }),
        { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }

    const emailResult = await emailResponse.json();
    console.log('Enrollment rejection email sent successfully:', emailResult.id);

    return new Response(
      JSON.stringify({
        success: true,
        message: `Enrollment rejection email sent to ${parentData.email}`,
    ...
    console.error('Error in send-enrollment-rejection:', error);
    return new Response(
